...
describe("getSignUpButtonText()", () => {
  test("should return the correct text for the sign up button", () => {
    expect(getSignUpButtonText(mockTFunction as TFunction, true, false)).toBe(
      SignUpActionEnum.LOGIN,
    );
    expect(getSignUpButtonText(mockTFunction as TFunction, false, false)).toBe(
      SignUpActionEnum.SIGNUP,
    );
